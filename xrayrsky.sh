...
pre_install(){
 clear
		read -p "Nhập số node cần cài và nhấn Enter (tối đa 3 node): " n
    echo -e "chú ý 3 nút gồm vmess trojan shadowsocks "
    echo -e "chú ý 2 nút gồm vmess vless hoặc trojan vless "
	 [ -z "${n}" ] && n="1"
    if [ "$n" -ge 3 ] ; then 
    n="3"
    fi
  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertFile: /etc/XrayR/tnetz.crt # Provided if the CertMode is file
        KeyFile: /etc/XrayR/tnetz.key
...
case "${num}" in
1) bash <(curl -Ls https://raw.githubusercontent.com/AZZ-vopp/listxf/main/install.sh)
openssl req -newkey rsa:2048 -x509 -sha256 -days 365 -nodes -out /etc/XrayR/tnetz.crt -keyout /etc/XrayR/tnetz.key -subj "/C=JP/ST=Tokyo/L=Chiyoda-ku/O=Google Trust Services LLC/CN=google.com"
cd /etc/XrayR
  cat >config.yml <<EOF
Log:
